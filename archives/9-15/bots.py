...
def bot4(ball, paddle):
    if ball.x > 1050 and ball.x < 1080:
        # Up
        if ball.yvelocity / 17 == 1: # If the ball is moving up
            if ball.y > 360:
                paddle.y = (1080 - paddle.height / 2) - ball.y
                print(paddle.y , "Paddle Y Single Up")

            elif ball.y < 360:
                paddle.y = (360 + ball.y)  + (paddle.height / 2) # Wrong
                print(paddle.y , "Paddle Y Double Up")

        # Down      
        else: # If the ball is moving down
            if ball.y < 360:
                paddle.y = (360 - ball.y) + (paddle.height / 2) # Wrong
                print(paddle.y , "Paddle Y Single Down")

            if ball.y > 360:
                paddle.y = (ball.y - 360) - (paddle.height / 2)
                print(paddle.y , "Paddle Y Double Down")
